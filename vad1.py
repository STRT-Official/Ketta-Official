...
import recog_aud
from recog_aud import main

FORMAT = pyaudio.paInt16
CHANNELS = 1
RATE = 16000
CHUNK_DURATION_MS = 30       # supports 10, 20 and 30 (ms)
PADDING_DURATION_MS = 5000   # 1 sec jugement
...
try:
    while True:
        if Path('torecog').is_file():
            ...
            sfile = wave.open('r1.wav', 'w')
            sfile.setnchannels(2)
            sfile.setsampwidth(2)
            sfile.setframerate(8000)
            sfile.writeframesraw(raw_data)
            # close file
            sfile.close()
            f=open('recorded', "w")
            f.close()
